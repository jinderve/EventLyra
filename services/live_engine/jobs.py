...
from services.live_engine.audio import chunk_wav
from services.live_engine.runtime import ChunkJob, SharedRuntime
...
def enqueue_wav(
    ...
    window = settings.chunk_seconds if chunk_seconds is None else chunk_seconds
    chunk_dir = wav_path.parent / "chunks" / wav_path.stem
    pieces = chunk_wav(wav_path, chunk_dir, window, settings.sample_rate)
    ...
    try:
        with session.lock:
            ...
            session.status = "processing" if pieces else "ready"
        ...
        with session.lock:
            ...
            if error:
                ...
                for cue in cues:
                    ...
                    if cue.latency_ms is not None:
                        session.last_latency_ms = cue.latency_ms
                    if cue.queue_wait_ms is not None:
                        session.last_queue_wait_ms = cue.queue_wait_ms
                if cues:
                    session.last_cue_at = time.time()
